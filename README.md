...
2. Install Dependencies: 
   ...
   npm install
   ```
3. Set Up Environment Variables: Copy `backend/.env.example` to `backend/.env` and set `MONGODB_URI`, `JWT_SECRET`, and `STRIPE_SECRET_KEY`. Copy the frontend and admin `.env.example` files when using a non-default API URL.
4. Run the Application:
   - Start the customer app: `npm run dev` from `frontend`.
   - Start the admin app: `npm run dev` from `admin`.
   - Start the API: `npm run server` from `backend`.
...
Feel free to contribute to Swaadly by forking this repository, creating a new branch, and submitting a pull request. Please make sure to follow standard coding practices and add meaningful comments to your code.

Enjoy exploring the code and features of Swaadly, and feel free to reach out for any questions or suggestions!
